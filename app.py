# SCHOLARS HAVEN V3.0.2 - FORCE DB RESET + PASSWORD SYSTEM
...
def init_db():
    ...
    return BASE_CSS + f"<div class='container'><h1 style='color:green; text-align:center'>Database RESET ✅</h1><p style='text-align:center; font-size:18px'>Loaded {count} questions across 9 subjects. All old users deleted.</p><a href='/' style='background:#1a3b6d; color:white; padding:12px 20px; border-radius:8px;'>Go Home</a></div>"
...
def home():
    ...
    html = BASE_CSS + f"""<body class="home-body"><div class="container home-container"><h1>Welcome {user_name}</h1><p style="text-align:center">UTME CBT | 10 Questions | 3 Minutes | 1 ATTEMPT TOTAL</p><form method="POST" action="/start_quiz"><select name="subject" required><option value="" disabled selected>Select Subject</option>{subject_options}</select><button>Start Quiz</button></form><p style="text-align:center; margin-top:15px"><a href="/logout">Logout</a></p></div></body>"""
